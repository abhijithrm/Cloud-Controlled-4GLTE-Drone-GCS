class VideoStreamClient {
    ...
    activateStream() {
        ...
        }//websocket enables two way comm between front end and backend ws endpoint. Here onmessage event handler get fired continuously as we will be recieving continuos data from backend ws.Updated src image data to give a video feel
    }
...
const addMarker = function (location) {
    ...
    var marker = new google.maps.Marker({
        ...
        map: WORLD_MAP
    });//google maps Marker obj initialized

    var pointId = SELECTED_DRONE.addPoint(marker);//selected drone is Drone dto obj, function returns lat+lng string

    var contentString = renderMapPointDataComponent(pointId, DEFAULT_ALTITUDE, DEFAULT_SPEED);

    var infowindow = new google.maps.InfoWindow({
        content: contentString
    });//we have to set the html for the infowview that opens on clicking marker point
    ...
    marker.addListener('dragend', function (event) {
        marker.setPosition(event.latLng);//dragging marker
    });
}
